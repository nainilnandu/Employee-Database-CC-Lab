...
export default function Report() {
    ...
    const [employee_ID, setEmId] = useState()

    console.log(location.state);
    useEffect(async () => {
        try {
            const { data } = await axios.get(`${apiEndpoint}/get-user/${location.state.employee_ID}`);
    ...
    const getUser = async () => {
        try {
            const { data } = await axios.get(`${apiEndpoint}/get-salary/${employee_ID}`);
            ...
            <div>
                ...
                <div>
                    {fuser ? <table className='table'>
                        ...
                        <tr>
                            <td>Total Salary</td>
                            <td>{parseInt(fuser?.salary.yearlyBonus) + 12 * parseInt(fuser?.salary.monthlySalary)}</td>
                        </tr>
                    </table> : <></>}
                </div>
            </div>
